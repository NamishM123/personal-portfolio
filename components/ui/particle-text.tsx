...
interface ParticleTextProps {
  ...
  fontFamily?: string;
  className?: string;
  /** When true, the canvas renders on a transparent background so it can be layered over other content. */
  transparent?: boolean;
...
const postFragmentShader = `
  ...
  void main() {
    ...
    // When layering over other content, derive alpha from luminance so the
    // glow fades to transparent instead of painting an opaque background.
    float luma = max(final.r, max(final.g, final.b));
    final.a = mix(1.0, clamp(luma, 0.0, 1.0), u_alphaFromLuma);
